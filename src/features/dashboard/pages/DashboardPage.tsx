    ...
    <div style={styles.shell}>
      ...
      <main style={styles.main}>
        ...
        <section style={styles.statsGrid}>
          {[
            { icon: <IconZap />,      label: 'Compilaciones',   value: '0',          sub: 'Esta sesión',       color: 'var(--accent)' },
            { icon: <IconActivity />, label: 'Estado del rover', value: 'En espera',  sub: 'Sin instrucciones', color: 'var(--accent3)' },
            { icon: <IconShield />,   label: 'Rol',             value: user?.rol ?? '—', sub: 'Acceso actual', color: 'var(--accent2)' },
            { icon: <IconCpu />,      label: 'Motor',           value: 'UMG++ v2',   sub: 'Compilador activo', color: 'var(--warning)' },
          ].map((card) => (
        ...
        <section style={styles.bottomGrid}>
          <div style={styles.panel}>
            ...
            <div style={styles.actionsList}>
              {[
                { icon: <IconCode />, title: 'Abrir editor', desc: 'Escribe y compila código UMG++', color: 'var(--accent)', to: '/editor' },
                { icon: <IconUser />, title: 'Ver perfil', desc: 'Gestiona tu cuenta y preferencias', color: 'var(--accent2)', to: '/profile' },
                { icon: <IconZap />, title: 'Nueva compilación', desc: 'Inicia un nuevo programa rover', color: 'var(--accent3)', to: '/editor' },
              ].map((action) => (
                <button key={action.title} onClick={() => navigate(action.to)} style={styles.actionItem}>
            ...
            <div style={styles.systemList}>
              {[
                ...
                <div key={item.label} style={styles.systemRow}>
                  ...
                  <span style={{
                    ...styles.systemBadge,
                    background: item.ok ? 'rgba(52,211,153,0.12)' : 'rgba(251,191,36,0.12)',
                    color: item.ok ? 'var(--accent3)' : 'var(--warning)',
                    border: `1px solid ${item.ok ? 'rgba(52,211,153,0.3)' : 'rgba(251,191,36,0.3)'}`,
                  }}>{item.value}</span>
                </div>
              ))}
            </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  loadingScreen: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: 'var(--bg-deep)' },
  ...
  sidebarUserRole: { color: 'var(--text-muted)', fontSize: '0.7rem', whiteSpace: 'nowrap' },
  ...
  topbar: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '2rem' },
  topbarGreeting: { color: 'var(--text-muted)', fontSize: '0.875rem', margin: 0 },
  topbarName: { color: 'var(--text-primary)', fontSize: '1.75rem', fontWeight: '700', margin: '0.25rem 0 0', fontFamily: 'var(--font-brand)', letterSpacing: '1px' },
  topbarBadge: { display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.25)', color: 'var(--accent3)', borderRadius: '20px', padding: '0.4rem 1rem', fontSize: '0.8rem', fontWeight: '500' },
  statusDot: { width: '8px', height: '8px', borderRadius: '50%', background: 'var(--accent3)', boxShadow: '0 0 8px var(--accent3)' },
  statsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '1.5rem' },
  statCard: { display: 'flex', alignItems: 'center', gap: '1rem', background: 'var(--bg-card)', borderRadius: '12px', padding: '1.25rem', border: '1px solid' },
  statIcon: { width: '44px', height: '44px', borderRadius: '10px', background: 'var(--bg-elevated)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  statContent: { display: 'flex', flexDirection: 'column', gap: '0.15rem' },
  statLabel: { color: 'var(--text-muted)', fontSize: '0.72rem', fontWeight: '500', textTransform: 'uppercase', letterSpacing: '0.5px' },
  statValue: { fontSize: '1.25rem', fontWeight: '700', fontFamily: 'var(--font-mono)' },
  statSub: { color: 'var(--text-muted)', fontSize: '0.72rem' },
  bottomGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' },
  panel: { background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border)', padding: '1.5rem' },
  panelTitle: { color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '1.25rem', marginTop: 0 },
  actionsList: { display: 'flex', flexDirection: 'column', gap: '0.5rem' },
  actionItem: { display: 'flex', alignItems: 'center', gap: '1rem', background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: '10px', padding: '0.9rem 1rem', cursor: 'pointer', width: '100%', textAlign: 'left' },
  actionIcon: { width: '36px', height: '36px', borderRadius: '8px', background: 'var(--bg-panel)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  actionText: { display: 'flex', flexDirection: 'column', flex: 1 },
  actionTitle: { color: 'var(--text-primary)', fontSize: '0.875rem', fontWeight: '600' },
  actionDesc: { color: 'var(--text-muted)', fontSize: '0.75rem', marginTop: '0.1rem' },
  systemList: { display: 'flex', flexDirection: 'column', gap: '0.65rem', marginBottom: '1.25rem' },
  systemRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  systemLabel: { color: 'var(--text-secondary)', fontSize: '0.8rem' },
  systemBadge: { borderRadius: '20px', padding: '0.2rem 0.75rem', fontSize: '0.72rem', fontWeight: '500' },
  userInfoBox: { display: 'flex', alignItems: 'center', gap: '1rem', background: 'var(--bg-elevated)', borderRadius: '10px', padding: '1rem', border: '1px solid var(--border)', marginTop: '1rem' },
  avatarLarge: { width: '48px', height: '48px', borderRadius: '50%', background: 'linear-gradient(135deg, var(--accent), var(--accent2))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: '700', fontSize: '1.25rem', flexShrink: 0 },
  userInfoName: { color: 'var(--text-primary)', fontWeight: '600', margin: 0, fontSize: '0.9rem' },
  userInfoEmail: { color: 'var(--text-secondary)', margin: '0.15rem 0 0', fontSize: '0.8rem' },
  userInfoDate: { color: 'var(--text-muted)', margin: '0.15rem 0 0', fontSize: '0.75rem' },
};